...
from tensorflow.contrib.layers import conv2d, conv2d_transpose, layer_norm
...
def build_conv2d_part(input_tensor, layer_num=5, filters_base = 16):
    ...
    net = conv2d(input_tensor, filters_base, kernel_size=1, normalizer_fn=layer_norm)
    ...
    for i in range(1, layer_num+1):
        ...
        with tf.variable_scope(f'block-down-{i}'):
            ...
            net = conv2d(net, filters_base * 2**i, kernel_size=1, padding='SAME', normalizer_fn=layer_norm)
            net = conv2d(net, filters_base * 2**i, kernel_size=3, stride=2, padding='SAME', normalizer_fn=layer_norm)
...
def build_deconv2d_part(input_tensor, residuals):
    ...
    for i, residual in enumerate(reversed(residuals)):
        with tf.variable_scope(f'block-up-{len(residuals) - i}'):
            ...
            net = conv2d_transpose(net, residual.shape[-1], kernel_size=3, stride=2, normalizer_fn=layer_norm)
            net = conv2d(net, residual.shape[-1], kernel_size=1, normalizer_fn=layer_norm)
