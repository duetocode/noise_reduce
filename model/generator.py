...
def build_conv2d_part(input_tensor, layer_num=5, filters_base = 16):
    ...
    net = conv2d(input_tensor, filters_base, kernel_size=1, normalizer_fn=pixel_norm)
    ...
    for i in range(1, layer_num+1):
        ...
        with tf.variable_scope(f'block-down-{i}'):
            ...
            net = conv2d(net, filters_base * 2**i, kernel_size=1, padding='SAME', normalizer_fn=pixel_norm)
            net = conv2d(net, filters_base * 2**i, kernel_size=3, stride=2, padding='SAME', normalizer_fn=pixel_norm)
...
def build_deconv2d_part(input_tensor, residuals):
    ...
    for i, residual in enumerate(reversed(residuals)):
        with tf.variable_scope(f'block-up-{len(residuals) - i}'):
            ...
            net = conv2d_transpose(net, residual.shape[-1], kernel_size=3, stride=2, normalizer_fn=pixel_norm)
            net = conv2d(net, residual.shape[-1], kernel_size=1, normalizer_fn=pixel_norm)
            net = tf.image.resize_image_with_crop_or_pad(net, *residual.shape.as_list()[1:3])
            net += residual
    
    net = conv2d(net, 3, kernel_size=1, activation_fn=None, normalizer_fn=None)
    return net